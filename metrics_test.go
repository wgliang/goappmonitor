...
func TestGauge(t *testing.T) {
	Gauge("testint64", 101)
	if v := GetGaugeValue("testint64"); v-101 > 0.00001 || v-101 < -0.00001 {
		t.Errorf("expect value:0.0 but response:%f", v)
	}
	Gauge("testfloat64", 101.0)
	if v := GetGaugeValue("testfloat64"); v-101 > 0.00001 || v-101 < -0.00001 {
...
func TestHistogram(t *testing.T) {
	...
	if v := GetHistogramCount("testhistogram"); v != 3 {
...
func TestCounter(t *testing.T) {
	...
	if v := GetCounterCount("testhistogram"); v != 201 {
...
func TestSelf(t *testing.T) {
	...
	if v := GetGaugeValue("tesself1"); v-0 > 0.00001 && v-0 < -0.00001 {
		t.Errorf("expect value:0.0 but response:%f", v)
	}

	Meter("tesself1", 101)
	selfMeter("tesself2", 101)
	if v := GetMeterCount("tesself2"); v != 0 {
